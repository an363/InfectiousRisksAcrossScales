...
# Run with python3 7A... num_scenario vx_wind vy_wind isotropic_inhalation
...
for parameters in model_parameters.values():
	...
	Risks_ij[parameters.name]["ped_no_A"]= Risks_ij[parameters.name]["index"].apply(lambda t: t[0])
	Risks_ij[parameters.name]["ped_no_B"]= Risks_ij[parameters.name]["index"].apply(lambda t: t[1])
	...
	if ContagionAmidGroups:
		Risks_ij[parameters.name]["exp_risk_AtoB"]= Risks_ij[parameters.name]["sum_risk_AtoB"].apply(lambda x: 1. - exp(-x) ) # this is the mean infection state I_B of B after contacts with diseased A
	else: 
		# a diseased individual cannot infect members of his/her own group
		Risks_ij[parameters.name]["exp_risk_AtoB"]= Risks_ij[parameters.name].apply(lambda row: (1. - exp(-row["sum_risk_AtoB"])) if row["group_no_A"]!=row["group_no_B"] else 0.0, axis=1)
...
with open(SaveRisksFile,'w') as monfichier:
	...
	for parameters in model_parameters.values():
		...
		mypedestrians["Cbari_%s"%parameters.name] = mypedestrians.apply(lambda row: DELTAT / row["tau"] * (
																		(myrisks.loc[ (myrisks["ped_no_A"]==row["ped_no"]) ])["exp_risk_AtoB"].sum() 
																		),
																		axis=1)
			

		## LOWER BOUND
		mypedestrians["Clowi_%s"%parameters.name] = mypedestrians.apply(lambda row: get_lower_bound_sum(row,
																										myrisks, 
																										tau_together / DELTAT * row["Cbari_%s"%parameters.name]), axis=1)
